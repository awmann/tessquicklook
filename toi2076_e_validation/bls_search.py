...
# known planets b,c,d periods to mask/note (TOI-2076 has 3 known planets before e)
known = {'b': 10.355, 'c': 17.166, 'd': 35.136}  # approx literature periods, for reference on plot only

durations = np.linspace(0.04, 0.15, 12)  # 1-3.6 hr grid around expected 0.09d
periods = np.linspace(1.6, 9.0, 6000)

results = {}
for name, t, f in [("SAP", t_sap, sap_res), ("PDCSAP", t_sap, pdc_res), ("custom", t_cus, cus_res)]:
    m = np.isfinite(f)
    tt, ff = t[m], f[m]
    ...
    pbest = periods[ibest]
    # stat at/near true period of planet e
    inear = np.argmin(np.abs(periods - P_true))
    window = np.abs(periods - periods[inear]) < 0.05
    ipeak_near = np.where(window)[0][np.argmax(power[window])]
    results[name] = dict(periods=periods, power=power, sde=sde,
                          global_best_P=pbest, global_best_sde=sde[ibest],
                          global_best_depth=r.depth[ibest],
                          near_e_P=periods[ipeak_near], near_e_sde=sde[ipeak_near],
                          near_e_depth=r.depth[ipeak_near], near_e_t0=r.transit_time[ipeak_near])
    print(f"\n{name}:")
    print(f"  global best peak: P={pbest:.4f} d  SDE={sde[ibest]:.2f}  depth={r.depth[ibest]*1e6:.0f} ppm")
    print(f"  peak nearest planet-e period (3.0223 d): P={periods[ipeak_near]:.4f} d  SDE={sde[ipeak_near]:.2f}  depth={r.depth[ipeak_near]*1e6:.0f} ppm")
    # rank of the near-e peak among all periods (is 3.02d region special?)
    rank = (power > power[ipeak_near]).sum()
    print(f"  rank of that peak among {len(periods)} trial periods: {rank} periods have higher power")

np.savez(f"{WD}/bls_results.npz",
         **{f"{k}_periods": v['periods'] for k,v in results.items()},
         **{f"{k}_power": v['power'] for k,v in results.items()},
         **{f"{k}_sde": v['sde'] for k,v in results.items()})
...
for ax, name in zip(axes, ["SAP","PDCSAP","custom"]):
    ...
    for kname, kp in known.items():
        ax.axvline(kp, color='gray', lw=0.8, ls=':', alpha=0.6)
    ax.set_title(f"{name}: BLS periodogram (0.75-day detrended, all 4 sectors)")
    ax.set_ylabel('SDE')
    ax.legend(fontsize=8, loc='upper right')
axes[-1].set_xlabel('trial period (days)')
fig.savefig(f"{WD}/toi2076_bls_periodogram.png", dpi=150)
print("\nsaved periodogram plot")
